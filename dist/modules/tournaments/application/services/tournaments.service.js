...
let TournamentsService = TournamentsService_1 = class TournamentsService {
    constructor(tournamentsRepository, usersService, achievementsService, prisma) {
     ...
     */
    async checkTournamentStreaks(userId) {
        try {
            ...
            const recentTournaments = await this.prisma.tournament.findMany({
                where: {
                    ...
                    status: tournament_enum_1.TournamentStatus.COMPLETED
                },
            ...
            for (const tournament of recentTournaments) {
                const formatDetails = tournament.formatDetails || {};
                const winners = formatDetails?.winners || [];
     ...
     */
    async checkMonthlyTournamentAchievements(userId) {
        try {

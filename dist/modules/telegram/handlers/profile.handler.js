...
let ProfileHandler = ProfileHandler_1 = class ProfileHandler {
    ...
    async handleSetupProfileAction(ctx) {
        try {
            ...
            this.stateService.setUserState(userId, {
                step: profile_state_enum_1.ProfileStep.AWAITING_SPORT_TYPE,
                data: {}
            });
            // Запрашиваем выбор спорта
            await ctx.reply('🎾 **ШАГ 1: Основные данные игрока**\n\nДавайте настроим ваш профиль! Сначала выберите спорт:', telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🎾 Теннис', 'sport_TENNIS'),
                    telegraf_1.Markup.button.callback('🏓 Падел', 'sport_PADEL')
                ]
            ]));
        }
        catch (error) {
    ...
    async handleTournamentsSelection(participates, ctx) {
        try {
            ...
            await ctx.reply(`✅ Участие в турнирах: ${participates ? 'Да' : 'Нет'}\n\n🎉 **ШАГ 1 ЗАВЕРШЁН!**\n\nОсновные данные сохранены. Теперь перейдём к настройке стиля игры и определению вашего рейтинга NTRP.`, telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('➡️ Перейти к Шагу 2', 'start_step_two')]
            ]));
        }
        catch (error) {
    ...
    async handleHandSelection(hand, ctx) {
        try {
            ...
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_PLAY_TIME;
            this.stateService.setUserState(userId, userState);
            ...
            await ctx.reply(`✅ Доминирующая рука: **${hand === 'LEFT' ? 'Левая' : 'Правая'}**\n\n🕐 Когда вы чаще всего играете? (можно выбрать несколько вариантов)`, {
                ...
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [
                        telegraf_1.Markup.button.callback('🌅 Утром', 'time_MORNING'),
                        telegraf_1.Markup.button.callback('☀️ Днём', 'time_DAY')
                    ],
                    [
                        telegraf_1.Markup.button.callback('🌇 Вечером', 'time_EVENING'),
                        telegraf_1.Markup.button.callback('🌙 Ночью', 'time_NIGHT')
                    ],
                    [
                        telegraf_1.Markup.button.callback('➡️ Продолжить', 'continue_to_frequency')
                    ]
                ]).reply_markup
            });
...
`;
     ...
     */
    async handleSportSelection(sportType, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_SPORT_TYPE) {
                return;
            }
            // Сохраняем выбор спорта
            userState.data.sportType = sportType;
     ...
     */
    async handlePlayTimeSelection(timeSlot, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_PLAY_TIME) {
                return;
            }
            ...
            const selectedTimes = userState.data.preferredPlayTime.map(t => timeNames[t]).join(', ');
            await ctx.reply(`✅ Время игры: ${selectedTimes || 'не выбрано'}\n\n🕐 Выберите ещё время или продолжите:`, telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🌅 Утром', 'time_MORNING'),
                    telegraf_1.Markup.button.callback('☀️ Днём', 'time_DAY')
                ],
    ...
    async handleContinueToFrequency(ctx) {
        try {
            ...
            this.stateService.setUserState(userId, userState);
            await ctx.reply('🏃‍♂️ Как часто вы играете?', telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('1 раз в неделю', 'frequency_1')],
                [telegraf_1.Markup.button.callback('2-3 раза в неделю', 'frequency_2')],
                [telegraf_1.Markup.button.callback('4+ раз в неделю', 'frequency_3')]
            ]));
        }
        catch (error) {
     ...
     */
    async handleBackhandSelection(backhandType, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_BACKHAND) {
                return;
            }
            userState.data.backhandType = backhandType;
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_SURFACE;
            this.stateService.setUserState(userId, userState);
            ...
            await ctx.reply(`✅ Бэкхенд: ${backhandName}\n\n🏟️ Какое покрытие вы предпочитаете?`, telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🔴 Хард', 'surface_HARD'),
                    telegraf_1.Markup.button.callback('🟤 Грунт', 'surface_CLAY')
                ],
                [
                    telegraf_1.Markup.button.callback('🟢 Трава', 'surface_GRASS'),
                    telegraf_1.Markup.button.callback('🔵 Ковер', 'surface_CARPET')
                ]
            ]));
        }
        catch (error) {
     ...
     */
    async handleSurfaceSelection(surface, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_SURFACE) {
                return;
            }
            userState.data.preferredSurface = surface;
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_STYLE;
            this.stateService.setUserState(userId, userState);
            const surfaceNames = {
                'HARD': 'хард',
                'CLAY': 'грунт',
            ...
            this.logger.log(`Выбрано покрытие: ${surfaceNames[surface]} для пользователя ${userId}`);
            // Переходим к выбору стиля игры
            await ctx.reply(`✅ Покрытие: ${surfaceNames[surface]}\n\n🎮 Какой у вас стиль игры?`, telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('🎯 Универсальный', 'style_UNIVERSAL')],
                [telegraf_1.Markup.button.callback('🛡️ Защитный', 'style_DEFENSIVE')],
                [telegraf_1.Markup.button.callback('⚡ Агрессивный с задней линии', 'style_AGGRESSIVE')],
                [telegraf_1.Markup.button.callback('🏐 Сеточник', 'style_NET_PLAYER')],
                [telegraf_1.Markup.button.callback('📚 Базовый', 'style_BASIC')]
            ]));
        }
        catch (error) {
     ...
     */
    async handleStyleSelection(style, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_STYLE) {
                return;
            }
            userState.data.playingStyle = style;
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_SHOT;
            this.stateService.setUserState(userId, userState);
            const styleNames = {
            ...
            this.logger.log(`Выбран стиль: ${styleNames[style]} для пользователя ${userId}`);
            // Переходим к выбору любимого удара
            await ctx.reply(`✅ Стиль: ${styleNames[style]}\n\n🎾 Какой ваш любимый удар?`, telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🚀 Подача', 'shot_SERVE'),
                    telegraf_1.Markup.button.callback('💪 Форхенд', 'shot_FOREHAND')
                ],
                [
                    telegraf_1.Markup.button.callback('🎯 Бэкхенд', 'shot_BACKHAND'),
                    telegraf_1.Markup.button.callback('🏐 Слёт', 'shot_VOLLEY')
                ],
                [telegraf_1.Markup.button.callback('⚡ Смэш', 'shot_SMASH')]
            ]));
        }
        catch (error) {
     ...
     */
    async handleShotSelection(shot, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_SHOT) {
                return;
            }
            userState.data.favoriteShot = shot;
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_RACKET;
            this.stateService.setUserState(userId, userState);
            const shotNames = {
                'SERVE': 'подача',
                'FOREHAND': 'форхенд',
            ...
            this.logger.log(`Выбран удар: ${shotNames[shot]} для пользователя ${userId}`);
            // Переходим к вводу ракетки
            await ctx.reply(`✅ Любимый удар: ${shotNames[shot]}\n\n🎾 Какой ракеткой вы играете? (напишите модель или "любая")`);
        }
        catch (error) {
     ...
     */
    async handleOpponentSelection(preference, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            const userId = ctx.from.id.toString();
            const userState = this.stateService.getUserState(userId);
            if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_OPPONENT_PREF) {
                return;
            }
            userState.data.opponentPreference = preference;
            userState.step = profile_state_enum_1.ProfileStep.COMPLETE;
            this.stateService.setUserState(userId, userState);
            ...
            // Отображаем сообщение об успешном завершении
            await ctx.reply(`✅ Предпочтения по сопернику: ${preferenceNames[preference]}\n\n🎉 **Профиль успешно настроен!**\n\nТеперь система может присвоить вам начальный рейтинг и подбирать подходящих соперников.\n\nВы можете пользоваться всеми функциями приложения!`, telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('👤 Перейти в профиль', 'profile')],
                [telegraf_1.Markup.button.callback('🎮 Главное меню', 'main_menu')]
            ]));
        }
        ...
        try {
            switch (userState.step) {
                ...
                case profile_state_enum_1.ProfileStep.AWAITING_COURT:
                    ...
                    break;
                case profile_state_enum_1.ProfileStep.AWAITING_RACKET:
     ...
     */
    async handleCityInput(city, ctx) {
        if (!ctx.from)
            return;
        const userId = ctx.from.id.toString();
        const userState = this.stateService.getUserState(userId);
        if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_CITY)
            return;
     ...
     */
    async handleCourtInput(court, ctx) {
        if (!ctx.from)
            return;
        const userId = ctx.from.id.toString();
        const userState = this.stateService.getUserState(userId);
        if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_COURT)
            return;
        userState.data.preferredCourt = court;
        userState.step = profile_state_enum_1.ProfileStep.AWAITING_HAND;
        this.stateService.setUserState(userId, userState);
        await ctx.reply(`✅ Предпочитаемый корт: ${court}\n\n🤚 Какой рукой вы играете?`, telegraf_1.Markup.inlineKeyboard([
            [
                telegraf_1.Markup.button.callback('👈 Левой', 'hand_LEFT'),
                telegraf_1.Markup.button.callback('👉 Правой', 'hand_RIGHT')
            ]
        ]));
    }
    /**
     ...
     */
    async handleRacketInput(racket, ctx) {
        if (!ctx.from)
            return;
        const userId = ctx.from.id.toString();
        const userState = this.stateService.getUserState(userId);
        if (userState.step !== profile_state_enum_1.ProfileStep.AWAITING_RACKET)
            return;
        userState.data.racket = racket;
        userState.step = profile_state_enum_1.ProfileStep.AWAITING_OPPONENT_PREF;
        this.stateService.setUserState(userId, userState);
        await ctx.reply(`✅ Ракетка: ${racket}\n\n👥 С кем вы предпочитаете играть?`, telegraf_1.Markup.inlineKeyboard([
            [
                telegraf_1.Markup.button.callback('🤷 Без разницы', 'opponent_ANY'),
                telegraf_1.Markup.button.callback('👨 Только с мужчинами', 'opponent_MEN')
            ],
            [
                telegraf_1.Markup.button.callback('👩 Только с женщинами', 'opponent_WOMEN'),
                telegraf_1.Markup.button.callback('⚖️ Похожий уровень', 'opponent_SAME_LEVEL')
            ],
            [
                telegraf_1.Markup.button.callback('💪 Сильнее меня', 'opponent_STRONGER'),
                telegraf_1.Markup.button.callback('🎯 Слабее меня', 'opponent_WEAKER')
            ]
        ]));
    }
    /**
     ...
     */
    async completeProfileSetupNew(telegramUserId, profileData) {
        try {
    ...
    async handleStartStepTwo(ctx) {
        try {
            ...
            this.stateService.setUserState(userId, userState);
            await ctx.reply('🥈 **ШАГ 2: Стиль игры и уровень**\n\nТеперь давайте определим ваш уровень игры для точного рейтинга NTRP.\n\nКакой у вас уровень игры?', telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('🔰 Новичок (1.0-2.0)', 'level_BEGINNER_1_2')],
                [telegraf_1.Markup.button.callback('🎾 Любитель (2.5-3.5)', 'level_AMATEUR_2_3')],
                [telegraf_1.Markup.button.callback('💪 Уверенный игрок (4.0-4.5)', 'level_CONFIDENT_4')],
                [telegraf_1.Markup.button.callback('🏆 Турнирный уровень (5.0+)', 'level_TOURNAMENT_5')],
                [telegraf_1.Markup.button.callback('👨‍🏫 Полупрофи/тренер', 'level_SEMI_PRO')]
            ]));
        }
        catch (error) {
     ...
     */
    async handleLevelSelectionStepTwo(level, ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
            ...
            userState.data.selfAssessedLevel = level;
            userState.step = profile_state_enum_1.ProfileStep.AWAITING_BACKHAND;
            this.stateService.setUserState(userId, userState);
            const levelNames = {
            ...
            this.logger.log(`Выбран уровень: ${levelNames[level]} для пользователя ${userId}`);
            // Переходим к выбору бэкхенда
            await ctx.reply(`✅ Уровень: ${levelNames[level]}\n\n🎾 Какой у вас бэкхенд?`, telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🤚 Одноручный', 'backhand_ONE'),
                    telegraf_1.Markup.button.callback('🙌 Двуручный', 'backhand_TWO')
                ]
            ]));
        }
        catch (error) {

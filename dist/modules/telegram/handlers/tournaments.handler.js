...
let TournamentsHandler = TournamentsHandler_1 = class TournamentsHandler {
    constructor(stateService, keyboardService, usersService, tournamentsService) {
    ...
    async handleTournaments(ctx) {
        try {
            ...
            if (userTournaments && userTournaments.length > 0) {
                ...
                userTournaments.slice(0, 5).forEach((tournament, index) => {
            ...
            const keyboard = telegraf_1.Markup.inlineKeyboard([
                ...
                [telegraf_1.Markup.button.callback('⬅️ Назад', 'main_menu')]
            ]);
    ...
    async handleMyTournaments(ctx) {
        try {
            ...
            const upcomingTournaments = allTournaments.filter((t) => t.status === 'DRAFT');
            const finishedTournaments = allTournaments.filter((t) => t.status === 'COMPLETED');
            ...
            if (finishedTournaments.length > 0) {
                ...
                finishedTournaments.slice(0, 3).forEach((tournament) => {
                    message += `• ${tournament.title}\n`;
                });
            ...
            await ctx.editMessageText(message, {
                ...
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('⬅️ Назад к турнирам', 'back_to_tournaments')]
                ]).reply_markup
            });
        }
        catch (error) {
    ...
    async handleCreateTournament(ctx) {
        try {
            ...
            this.stateService.setUserState(userId, {
                step: profile_state_enum_1.ProfileStep.CREATING_TOURNAMENT,
                data: {}
            });
    ...
    async handleFindTournament(ctx) {
        try {
            ...
            if (availableTournaments && availableTournaments.length > 0) {
                ...
                availableTournaments.slice(0, 5).forEach((tournament, index) => {
                ...
                const buttons = [];
                availableTournaments.slice(0, 3).forEach((tournament) => {
                    buttons.push([
                        telegraf_1.Markup.button.callback(`Присоединиться к "${tournament.title}"`, `join_tournament_${tournament.id}`)
                    ]);
                });
    ...
    async handleViewTournament(ctx) {
        try {
            await ctx.answerCbQuery();
            ...
            if (tournament.status === 'DRAFT') {
                buttons.push([
                    telegraf_1.Markup.button.callback('✅ Присоединиться', `join_tournament_${tournament.id}`),
                    telegraf_1.Markup.button.callback('❌ Покинуть', `leave_tournament_${tournament.id}`)
                ]);
            }
    ...
    async handleLeaveSpecificTournament(ctx) {
        try {
            await ctx.answerCbQuery();
            if (!ctx.from)
                return;
    ...
    async handleTournamentInput(ctx, text, userId) {
        ...
        switch (userState.step) {
            case profile_state_enum_1.ProfileStep.CREATING_TOURNAMENT:
            ...
            case profile_state_enum_1.ProfileStep.AWAITING_TOURNAMENT_START_DATE:
                return await this.handleTournamentStartDate(ctx, text, userId, userState);
            case profile_state_enum_1.ProfileStep.AWAITING_TOURNAMENT_MAX_PARTICIPANTS:
    ...
    async handleTournamentDescription(ctx, text, userId, userState) {
        ...
        userState.step = profile_state_enum_1.ProfileStep.AWAITING_TOURNAMENT_START_DATE;
        this.stateService.setUserState(userId, userState);
        await ctx.reply('📅 Введите дату начала турнира (в формате ДД.ММ.ГГГГ):');
        return true;
    }
    async handleTournamentStartDate(ctx, text, userId, userState) {
        ...
        userState.step = profile_state_enum_1.ProfileStep.AWAITING_TOURNAMENT_MAX_PARTICIPANTS;
        this.stateService.setUserState(userId, userState);
        await ctx.reply('👥 Введите максимальное количество участников (от 4 до 32):');
        return true;
    }
    async handleTournamentMaxParticipants(ctx, text, userId, userState) {
        ...
        try {
            ...
            const tournamentDto = {
                ...
                type: tournament_enum_1.TournamentType.SINGLE_ELIMINATION, // По умолчанию
                isRanked: true,
            ...
            await ctx.reply(`✅ **Турнир "${tournament.title}" создан!**\n\n` +
                `📅 Дата: ${new Date(tournament.startDate).toLocaleDateString('ru-RU')}\n` +
                `👥 Максимум участников: ${tournament.maxPlayers}\n\n` +
                `Турнир будет доступен для регистрации других игроков.`, { parse_mode: 'Markdown' });
            // Сбрасываем состояние
            this.stateService.setUserState(userId, { step: profile_state_enum_1.ProfileStep.IDLE, data: {} });
        }
        catch (error) {

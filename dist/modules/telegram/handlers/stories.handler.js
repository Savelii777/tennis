...
const profile_state_enum_1 = require("../interfaces/profile-state.enum"); // Обновляем путь при необходимости
let StoriesHandler = StoriesHandler_1 = class StoriesHandler {
    constructor(stateService, keyboardService, storiesService, usersService) {
    ...
    async handleStories(ctx) {
        try {
            const keyboard = telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('📤 Загрузить Story', 'create_story')],
                [telegraf_1.Markup.button.callback('📋 Мои Stories', 'my_stories')],
                [telegraf_1.Markup.button.callback('🔥 Популярные', 'popular_stories')],
                [telegraf_1.Markup.button.callback('🕐 Недавние', 'recent_stories')],
    ...
    async handleCreateStory(ctx) {
        try {
            ...
            await ctx.reply('📸 Отправьте фото или видео (до 30 секунд) для создания истории.\n\n' +
    ...
    async handlePhoto(ctx) {
        try {
            ...
            if (userState.page === 'stories') {
                ...
                const updatedUserState = {
                    ...userState,
                    step: profile_state_enum_1.ProfileStep.AWAITING_STORY_DESCRIPTION,
                    data: {
                        ...userState.data,
                        fileId: photo.file_id,
                        fileType: client_1.MediaType.image
                    }
                };
                this.stateService.setUserState(userId, updatedUserState);
                await ctx.reply('📝 Отлично! Теперь введите описание для вашей Story (или отправьте "-" для публикации без описания):');
                return true;
            }
            return false;
        }
            ...
            if (userState.page === 'stories') {
                ...
                const updatedUserState = {
                    ...
                    data: {
                        ...
                        fileType: client_1.MediaType.video
                    }
                };
                this.stateService.setUserState(userId, updatedUserState);
    ...
    async handleStoryInput(ctx, text, userId) {
        try {
            ...
            if (userState.step === profile_state_enum_1.ProfileStep.AWAITING_STORY_DESCRIPTION && userState.data?.fileId) {
                // Находим пользователя
                const user = await this.usersService.findByTelegramId(userId);
                if (!user) {
                    await ctx.reply('❌ Пользователь не найден. Пожалуйста, выполните /start');
                ...
                await ctx.reply('✅ Ваша Story успешно создана и отправлена на модерацию!\n\n' +
                    'После проверки модератором она станет доступна в разделе Stories.', {
                    reply_markup: telegraf_1.Markup.inlineKeyboard([
                        [telegraf_1.Markup.button.callback('📋 Мои Stories', 'my_stories')],
                        [telegraf_1.Markup.button.callback('📤 Загрузить еще', 'create_story')],
                        [telegraf_1.Markup.button.callback('🔙 Назад', 'back_to_stories')]
                    ]).reply_markup
                });
                return true;
            }
    ...
    async handleMyStories(ctx) {
        try {
            ...
            if (stories.length === 0) {
                await ctx.reply('📱 У вас пока нет Stories\n\n' +
            ...
            await ctx.reply(message, {
                parse_mode: 'Markdown',
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('📤 Создать новую Story', 'create_story')],
                    [telegraf_1.Markup.button.callback('🔙 Назад', 'back_to_stories')]
                ]).reply_markup
            });
        }
    ...
    async handlePopularStories(ctx) {
        try {
            ...
            await ctx.reply(message, {
                parse_mode: 'Markdown',
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('📤 Создать свою Story', 'create_story')],
                    [telegraf_1.Markup.button.callback('🔙 Назад', 'back_to_stories')]
                ]).reply_markup
            });
        }
    ...
    async handleBackToStories(ctx) {
        return this.handleStories(ctx);
    }
};
exports.StoriesHandler = StoriesHandler;
exports.StoriesHandler = StoriesHandler = StoriesHandler_1 = __decorate([
